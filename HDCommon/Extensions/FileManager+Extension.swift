...
extension FileManager: HDExtended {}
...
public extension HDExtension where ExtendedType: FileManager {
...
public extension HDExtension where ExtendedType: FileManager {
    ...
    enum FileManagerError: Error {
        ...
        case wrongPath
        /// 创建路径失败
        case creatPathFailure
        /// 删除路径失败
        case removePathFailure
        /// 写入失败
        case writeFailure
        /// data 为空
        case nilData
    ...
    @discardableResult
    static func createFolder(folderPath: String) -> Result<Bool, FileManagerError> {
        ...
        do {
            ...
            return .failure(.creatPathFailure)
        }
    }
    ...
    @discardableResult
    static func removefolder(folderPath: String) -> Result<Bool, FileManagerError> {
        ...
        do {
            ...
            return .failure(.removePathFailure)
        }
    }
    ...
    @discardableResult
    static func createFile(filePath: String) -> Result<Bool, FileManagerError> {
        guard judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径才会创建
    ...
    /// - Parameter filePath: 文件路径
    @discardableResult
    static func removefile(filePath: String) -> Result<Bool, FileManagerError> {
        guard judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return .success(true)
        }
    ...
    static func readfile(filePath: String) -> String? {
        guard let data = readFileData(filePath: filePath) else { return nil }
        return String(data: data, encoding: String.Encoding.utf8)
    }
    
    static func readFileData(filePath: String) -> Data? {
        guard judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return nil
        }
    ...
    /// - Returns: 写入的结果
    @discardableResult
    static func writeToFile(writeType: FileWriteType, content: Any, writePath: String) -> Result<Bool, FileManagerError> {
        guard judgeFileOrFolderExists(filePath: directoryAtPath(path: writePath)) else {
            // 不存在的文件路径
            return .failure(.nilPath)
        }
    ...
    /// - Returns: 返回读取的内容
    @discardableResult
    static func readFromFile(readType: FileWriteType, readPath: String) -> Result<Any?, FileManagerError> {
        ...
        case .ArrayType:
            ...
            return .success(readString.hd.jsonStringToArray())
            
        case .DictionaryType:
            guard let readString = String(data: data, encoding: String.Encoding.utf8) else {
                return .failure(.nilData)
            }
            return .success(readString.hd.jsonStringToDictionary())
        }
    }
    ...
    /// - Returns: 拷贝的结果
    @discardableResult
    static func copyFile(type: MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> Result<Bool, FileManagerError> {
        ...
        let isExistFolder = judgeFileOrFolderExists(filePath: toFileFolderPath)
        if isExistFolder == false {
            if type == .file {
                let createFileResult = createFile(filePath: toFilePath)
                ...
                let createFolderResult = createFolder(folderPath: toFileFolderPath)
    ...
    ///   - toFile: 移动后的文件路径
    @discardableResult
    static func moveFile(type: MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> Result<Bool, FileManagerError> {
        // 1、先判断被拷贝路径是否存在
        guard judgeFileOrFolderExists(filePath: fromeFilePath) else {
            return .failure(.nilPath)
        }
...
public extension HDExtension where ExtendedType: FileManager {
    ...
    /// - Returns: 返回获取的图片
    private static func getVideoImage(videoUrlSouceType: HDVideoUrlType = .local, path: String, seconds: Double = 1, preferredTimescale: CMTimeScale = 10, maximumSize: CGSize?, preferredTrackTransform: Bool = true) -> UIImage? {
        var videoURL: URL?
