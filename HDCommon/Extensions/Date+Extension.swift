...
extension Date: HDExtended {}

/// 时间戳的类型
public enum HDTimestampType: Int {
    /// 秒
    case second
    /// 毫秒
    case millisecond
...
// MARK: - 一、Date 基本的扩展
public extension HDExtension where ExtendedType == Date {
    
    // MARK: 1.1、获取当前 秒级 时间戳 - 10 位
    /// 获取当前 秒级 时间戳 - 10 位
    static var secondStamp : String {
        let timeInterval: TimeInterval = ExtendedType().timeIntervalSince1970
    ...
    /// 从 Date 获取年份
    var year: Int {
        return Calendar.current.component(Calendar.Component.year, from: self.type)
    }
    
    // MARK: 1.5、从 Date 获取月份
    /// 从 Date 获取年份
    var month: Int {
        return Calendar.current.component(Calendar.Component.month, from: self.type)
    }
    
    // MARK: 1.6、从 Date 获取 日
    /// 从 Date 获取 日
    var day: Int {
        return Calendar.current.component(.day, from: self.type)
    }
    
    // MARK: 1.7、从 Date 获取 小时
    /// 从 Date 获取 日
    var hour: Int {
        return Calendar.current.component(.hour, from: self.type)
    }
    
    // MARK: 1.8、从 Date 获取 分钟
    /// 从 Date 获取 分钟
    var minute: Int {
        return Calendar.current.component(.minute, from: self.type)
    }
    
    // MARK: 1.9、从 Date 获取 秒
    /// 从 Date 获取 秒
    var second: Int {
        return Calendar.current.component(.second, from: self.type)
    }
    
    // MARK: 1.10、从 Date 获取 毫秒
    /// 从 Date 获取 毫秒
    var nanosecond: Int {
        return Calendar.current.component(.nanosecond, from: self.type)
    }
    ...
    var weekday: String {
        ...
        return DateFormatter.shared.string(from: self.type)
    }
    ...
    var weekdayStringFromDate: String {
        ...
        let theComponents = calendar.dateComponents([.weekday], from: self.type as Date)
    ...
    var monthAsString: String {
        DateFormatter.shared.dateFormat = "MM"
        return DateFormatter.shared.string(from: self.type)
    }
}
...
public extension HDExtension where ExtendedType == Date {
    ...
    func toformatterTimeString(formatter: String = "yyyy-MM-dd HH:mm:ss") -> String {
        ...
        DateFormatter.shared.timeZone = TimeZone.autoupdatingCurrent
    ...
    /// - Returns: 返回转化后的时间戳
    static func formatterTimeStringToTimestamp(timesString: String, formatter: String, timestampType: HDTimestampType = .second) -> String {
        DateFormatter.shared.dateFormat = formatter
        DateFormatter.shared.locale = NSLocale.system
        guard let date = DateFormatter.shared.date(from: timesString) else {
            #if DEBUG
            fatalError("时间有问题")
            #else
            return ""
    ...
    ///   - type: 格式类型
    /// - Returns: 返回时间条
    static func getFormatPlayTime(seconds: Int, type: HDTimeBarType = .normal) -> String {
        if seconds <= 0 {
            return "00:00"
        }
        // 秒
    ...
    /// - Returns: 时间戳
    func dateToTimeStamp(timestampType: HDTimestampType = .second) -> String {
        // 10位数时间戳 和 13位数时间戳
        let interval = timestampType == .second ? CLongLong(Int(self.type.timeIntervalSince1970)) : CLongLong(round(self.type.timeIntervalSince1970 * 1000))
        return "\(interval)"
    }
    
    // 转成当前时区的日期
    func dateFromGMT() -> Date {
        let secondFromGMT: TimeInterval = TimeInterval(TimeZone.current.secondsFromGMT(for: self.type))
        return self.type.addingTimeInterval(secondFromGMT)
    }
}
...
public extension HDExtension where ExtendedType == Date {
    ...
    func toLocalDateFormatterStyleString(locale: Locale = Locale.current, formatter: String = "yyyyMMMd", dateStyle: DateFormatter.Style = .short, timeStyle: DateFormatter.Style = .none, timeZone: TimeZone = TimeZone.autoupdatingCurrent) -> String {
        ...
        return dateFormatter.string(from: self.type)
    }
...
public extension HDExtension where ExtendedType == Date {
    ...
    /// - Returns: bool
    var isToday: Bool {
        return Calendar.current.isDate(self.type, inSameDayAs: Date())
    }
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = ExtendedType.hd.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.type, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = ExtendedType.hd.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.type, inSameDayAs: date)
    ...
    var isThisYear: Bool  {
        ...
        let selfCmps = calendar.dateComponents([.year], from: self.type)
    ...
    private func adding(day: Int) -> Date? {
        return Calendar.current.date(byAdding: DateComponents(day:day), to: self.type)
    }
    ...
    private func isSameYeaerMountDay(_ date: Date) -> Bool {
        let com = Calendar.current.dateComponents([.year, .month, .day], from: self.type)
    ...
    var isThisWeek: Bool {
        ...
        // self
        let selfComponents = calendar.dateComponents([.weekday,.month,.year], from: self.type as Date)
...
public extension HDExtension where ExtendedType == Date {
    ...
    /// 取得与当前时间的间隔差
    /// - Returns: 时间差
    func callTimeAfterNow() -> PastTime {
        let timeInterval = Date().timeIntervalSince(self.type)
        if timeInterval < 0 {
            return .justNow
        }
    ...
    func componentCompare(from date: Date, unit: Set<Calendar.Component> = [.year, .month, .day, .hour, .minute]) -> DateComponents {
        let calendar = Calendar.current
        let component = calendar.dateComponents(unit, from: date, to: type)
        return component
    }
...
public extension HDExtension where ExtendedType == Date {
    ...
    static var currentYear : Int {
        return currentDate.hd.year
    }
    
    // MARK: 6.2、今年是不是闰年
    /// 1.16、今年是不是闰年
    static var currentYearIsLeapYear: Bool {
        return todayDate.hd.isLeapYear
    }
